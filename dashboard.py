...
def check_password():
    if "password_correct" not in st.session_state:
        st.markdown('<div class="main-header">🔒 LOGIN DASHBOARD</div>', unsafe_allow_html=True)
...
if check_password():
    ...
    if raw_bytes:
        ...
        if page == "Dashboard Utama":
            st.markdown(f'<div class="main-header">DASHBOARD - {target_sheet}</div>', unsafe_allow_html=True)
            ...
            m2.metric("TOTAL SALES", format_rupiah(ts), delta=f"{get_delta_val(ts, avg_hist['sales']):.1f}%" if avg_hist['sales']>0 else None)
            m3.metric("TOTAL SHRINK", format_rupiah(tr), delta=f"{get_delta_val(tr, avg_hist['shrink']):.1f}%" if avg_hist['shrink']>0 else None, delta_color="inverse")
            m4.metric("% S/S", f"{(tr/ts*100 if ts>0 else 0):.2f}%")
            ...
            with c_main:
                ...
                with t1:
                    ...
                    s_tbl = s_grp.copy()
                    s_tbl['GROWTH'] = s_tbl[c_dept].apply(lambda x: format_growth_html(get_delta_val(s_tbl[s_tbl[c_dept]==x][c_sales].sum(), avg_hist["dept_sales"].get(x, 0))))
                    s_tbl[c_sales] = s_tbl[c_sales].apply(format_rupiah)
                    st.write(s_tbl.to_html(escape=False, index=False), unsafe_allow_html=True)
                with t2:
                    st.write("**Rincian Shrink Dept**")
                    r_tbl = r_grp.copy()
                    r_tbl['GROWTH'] = r_tbl[c_dept].apply(lambda x: format_growth_html(get_delta_val(r_tbl[r_tbl[c_dept]==x][c_shrink].sum(), avg_hist["dept_shrink"].get(x, 0)), inverse=True))
                    r_tbl[c_shrink] = r_tbl[c_shrink].apply(format_rupiah)
                    st.write(r_tbl.to_html(escape=False, index=False), unsafe_allow_html=True)
            ...
            with c_side:
                ...
                top_s[c_sales], top_s[c_qty_s] = top_s[c_sales].apply(format_rupiah), top_s[c_qty_s].apply(format_qty)
                st.write(top_s.to_html(escape=False, index=False), unsafe_allow_html=True)
                st.divider()
                ...
                top_r[c_shrink], top_r[c_qty_r] = top_r[c_shrink].apply(format_rupiah), top_r[c_qty_r].apply(format_qty)
                st.write(top_r.to_html(escape=False, index=False), unsafe_allow_html=True)
            ...
            with b1:
                ...
                # Menambahkan Shrink Value di sini agar tidak hilang
            ...
            with b2:
                ...
                # Menambahkan Shrink Value (c_shrink) agar muncul kembali
